...
class SetLocalCollection {
  ...
  static Future<void> init() async {
    ...
    final repDat = await specieData.document('reptiles').get();
    ...
    for (var i = 0; i < colDat.subTypeReptile.length; i++) {
      repFile.writeAsStringSync("${colDat.subTypeReptile[i]}\n",
          mode: FileMode.append);
      repFile.writeAsStringSync(
          "${(repDat[colDat.subTypeReptile[i]]).toString()}\n",
          mode: FileMode.append);
    }
